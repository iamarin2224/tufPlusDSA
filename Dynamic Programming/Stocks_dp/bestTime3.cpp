...
//? Memoization
int helper(vector<int> &arr, int i, int buy, int sellCount, vector<vector<vector<int>>> &dp){
    ...
    if (buy){
        profit = max(
            -arr[i]+helper(arr, i+1, 0, sellCount, dp), //buy the current and go to next with "not to buy"
            helper(arr, i+1, 1, sellCount, dp) //do not buy, and go to next with "can buy"
        );
    }
    else{
        profit = max(
            arr[i]+helper(arr, i+1, 1, sellCount+1, dp), //sell the current and go to next with "can buy"
            helper(arr, i+1, 0, sellCount, dp) //do not sell thus go to next telling "not to buy"
        );
    }
...
int stockBuySell(vector<int> arr, int n){
    ...
    for (int i=n-1; i>=0; i--){
        ...
        for (int buy=0; buy<=1; buy++){
            for (int sellCount=0; sellCount<=1; sellCount++){
                next[buy][sellCount] = cur[buy][sellCount];
            }
        }
    }
